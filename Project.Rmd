...
ggplot(my_df, aes(x = Gender, y = Trust, color = Gender)) +
  geom_jitter(width = 0.15, height = 0.15, alpha = 0.4, size = 2) +
  geom_point(
    data = trust_summary, 
    aes(x = Gender, y = median),  # Added x aesthetic
    shape = 21, size = 8, fill = "white", stroke = 1.5,
    inherit.aes = FALSE
  ) +
  geom_text(
    data = trust_summary,
    aes(x = Gender, y = median, label = median),  # Added x aesthetic
  ...
  labs(
    title = "Gender Differences in AI Trust",
    subtitle = "White circles show group medians",
    y = "Trust Level (1-5)"
  ) +
  theme_minimal()
